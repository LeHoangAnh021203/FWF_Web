...
const industryMeta = [
  { id: "budget", layoutClass: "lg:col-start-1 lg:row-start-1" },
  { id: "care", layoutClass: "lg:col-start-2 lg:row-start-1" },
  { id: "easy", layoutClass: "lg:col-start-1 lg:row-start-2" },
  { id: "pro", layoutClass: "lg:col-start-2 lg:row-start-2" },
] as const
...
export function WhyChooseSection() {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      setActiveIndex((current) => {
        ...
        if (next > showcaseImages.length) return current
        return next
      })
    }, 2600)
  ...
  useEffect(() => {
    ...
    const resetTimer = window.setTimeout(() => {
      ...
      setActiveIndex(0)
      window.setTimeout(() => setIsAnimating(true), 80)
    }, 700)
              ...
              >
                <summary className="flex min-h-[148px] cursor-pointer list-none flex-col justify-between rounded-[22px] border border-orange-100/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.65),rgba(255,247,239,0.38))] p-4 marker:hidden md:min-h-[164px] md:p-5 [&::-webkit-details-marker]:hidden">
                  ...
                  <h3 className="mt-4 max-w-[18ch] text-left text-lg font-bold leading-tight text-orange-950 md:text-[1.35rem]">
                    {t(`b2b.ind.${industry.id}.name`)}
                  </h3>
                </summary>
                <div className="px-2 pb-2 pt-4">
                  <p
                    className="text-[15px] leading-6 text-stone-600"
                    dangerouslySetInnerHTML={{ __html: t(`b2b.ind.${industry.id}.desc`) }}
                  />
                </div>
              </details>
            ))}
...
function IndustryIcon({ id }: { id: (typeof industryMeta)[number]["id"] }) {
  if (id === "budget") {
    return (
